...
// HT_DELETED_ITEM marks a bucket containing a deleted item
static ht_item HT_DELETED_ITEM = {NULL, NULL};

// Parameters for the hashing algorithim
static const int HT_PRIME_1 = 193;
static const int HT_PRIME_2 = 197;

static int HT_INITIAL_BASE_SIZE = 0;
...
/*
 * Initialises a new empty hash table using a set size index
 */
static ht_hash_table *ht_new_sized(const int base_size) {
  ht_hash_table *ht = xmalloc(sizeof(ht_hash_table));
  ht->base_size = base_size;

  const int size_index = 50 << ht->base_size;
  ht->size = next_prime(size_index);
...
static void ht_resize(ht_hash_table *ht, const int base_size) {
  ...
  // Create temp hash table to insert items to
  ...
  // To delete new_ht, pass down ht's size and items
...
/*
 * Returns the hash of 'string', an int between 0 and num_buckets
 */
static int ht_hash(const char *string, const int a, const int num_buckets) {
  long hash = 0;
  const int str_len = strlen(string);
  for (int i = 0; i < str_len; i++) {
    hash += (long)pow(a, str_len - (i + 1)) * string[i];
...
void ht_insert(ht_hash_table *ht, const char *key, const char *value) {
  ...
  // Cycle through filled buckets until we hit an empty or deleted one
